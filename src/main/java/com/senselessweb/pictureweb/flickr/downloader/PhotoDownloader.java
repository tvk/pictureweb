...
public class PhotoDownloader implements ApplicationListener<AuthenticationFinishedEvent> {
	...
	public void fetchUserPhotos(final int page) {
		...
		try {
			...
			final PhotoList<Photo> photos = flickr.get().getPeopleInterface().getPhotos(flickr.getUserId(), null, null,
					null, null, null, null, null, extra, 500, page);
			
			photos.stream()
				.map(p -> new StoredPhoto(context, p))
